"""
Spider detail cho ITviec.
- Đọc danh sách job_id/url từ data/raw/itviec/{batch_date}/jobs_meta_listing.jsonl
- Dùng Playwright (requires_browser=True) để tải trang chi tiết thật sự
  (URL đã được listing spider build đúng dạng /it-jobs/{slug}, không phải preview AJAX)
...
class ItviecDetailSpider(BaseSpider):
    ...
    async def start(self):
        ...
        if not listing_path.exists():
            self.logger.error(
                ...
                f"Hãy chạy 'scrapy crawl itviec_listing -a batch_date={self.batch_date}' trước."
            )
            return
        ...
        self.logger.info(f"start: yielded {count} requests")
    ...
    def parse(self, response):
        ...
        # Lấy title từ trang detail (thường nằm trong thẻ h1)
        title = response.css("h1::text").get()
        if not title:
            # Fallback nếu selector h1 thay đổi
